...
import cn.wolfcode.shop.utils.UploadUtil;
...
public class ProductController {
    ...
    public String getPropertyValueTr(Long catalogId,Map map){
        //通过分类id获取分类属性集合
        List<Property> propertyList = propertyService.getProByCatalogId(catalogId);
        //遍历该分类属性集合
        propertyList.forEach(property -> {
            //获取每一个分类属性的类型,如果是下拉列表类型,就再获取该分类属性的属性值集合
            if(property.getType() == 1){
                List<PropertyValue> propertyValueList = propertyValueService.getProValByProId(property.getId());
                //把该属性值的集合设置到分类属性中
        ...
        return "product/product_property_tr";
    }
